...
JS_TOGGLE = """(function(){var b=document.getElementById('pvtog');if(!b)return JSON.stringify({skip:true});var paint=function(){return [].slice.call(document.querySelectorAll('.card,.buildcard')).filter(function(c){return getComputedStyle(c).display!=='none'}).length};var stale=function(){var bad=[];document.querySelectorAll('details.cat').forEach(function(g){g.querySelectorAll('.tgroup').forEach(function(tg){var k=[].slice.call(tg.querySelectorAll('.card')).filter(function(c){return getComputedStyle(c).display!=='none'}).length;var strip=g.querySelector('.tgs .tg[data-g="'+tg.dataset.g+'"]');if(!strip)return;var shown=strip.hidden?0:parseInt(strip.querySelector('.cn').textContent,10);if(shown!==k)bad.push(tg.dataset.g+':'+shown+'!='+k);});});return bad};var hid=function(){return b.getAttribute('aria-pressed')==='true'};var wasHid=hid();if(wasHid)b.click();var before=paint();b.click();var after=paint();var l1=b.querySelector('.pvl');var lab1=l1?l1.textContent:'';var badStrip=stale();b.click();var l2=b.querySelector('.pvl');var lab2=l2?l2.textContent:'';if(wasHid)b.click();return JSON.stringify({before:before,after:after,total:document.querySelectorAll('.card,.buildcard').length,hasSwitch:!!l1&&!!l2&&lab1!==lab2,defaultHidden:wasHid,staleStrip:badStrip.slice(0,4)})})()"""
        ...
        for w, h in VIEWPORTS:
            ...
            if not r.get("skip"):
                ...
                if not r.get("hasSwitch"):
                    fail(w, "toggle", "the toggle's label (.pvl) vanished or did not "
