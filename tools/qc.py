...
JS_FEATURED = """(function(){var f=document.querySelector('.feat');if(!f)return JSON.stringify({skip:true});var paint=function(e){return getComputedStyle(e).display!=='none'};var cards=function(sel){return [].slice.call(f.querySelectorAll(sel)).filter(paint).length};var q=document.getElementById('q');var chip=[].slice.call(document.querySelectorAll('.chip')).filter(function(c){return c.dataset.f&&c.dataset.f!=='all'&&c.dataset.f.indexOf('kind:')!==0})[0];var o={dup:document.querySelectorAll('.fcard.card').length};o.initial=paint(f);q.value='transfer';q.dispatchEvent(new Event('input'));o.onSearch=paint(f);q.value='';q.dispatchEvent(new Event('input'));o.cleared=paint(f);if(chip){chip.click();o.onChip=paint(f);chip.click();o.chipOff=paint(f);}var t=document.getElementById('pvtog');if(t){var wasHid=!t.checked;if(wasHid)t.click();var before=cards('.fcard');o.plannedBefore=cards('.fcard.planned');t.click();o.togBand=paint(f);o.togShrank=cards('.fcard')<before;o.togPlanned=cards('.fcard.planned');t.click();if(wasHid)t.click();}return JSON.stringify(o)})()"""


JS_CAROUSEL = """(function(){var f=document.querySelector('.feat');if(!f)return JSON.stringify({skip:true});var t=f.querySelector('.ftrack');var cards=[].slice.call(f.querySelectorAll('.fcard'));if(!t||cards.length<2)return JSON.stringify({skip:true});var o={cards:cards.length};o.dots=f.querySelectorAll('.fdot').length;o.per=parseInt(t.style.getPropertyValue('--per'),10)||0;var w=f.querySelector('.fviewport');o.clipped=w?getComputedStyle(w).overflow==='hidden':false;var cw=cards[0].getBoundingClientRect().width;var vw=w?(w.clientWidth-parseFloat(getComputedStyle(w).paddingLeft)-parseFloat(getComputedStyle(w).paddingRight)):0;o.widthMatchesPer=(vw&&o.per)?Math.abs(cw-((vw-(o.per-1)*10)/o.per))<3:false;var live=cards.filter(function(c){return !c.hidden}).length;o.dotsMatch=o.per?o.dots===Math.ceil(live/o.per):false;var at=function(){return t.style.transform};var next=f.querySelector('.farrow[data-step="1"]');var prev=f.querySelector('.farrow[data-step="-1"]');var play=f.querySelector('.fplay');if(play)play.click();var was=at();next.click();o.moved=at()!==was;var m=/translateX\\(-?([0-9.]+)px\\)/.exec(at()||'');o.stepPx=m?parseFloat(m[1]):null;o.expectedStep=vw+10;o.status=(f.querySelector('.fstatus')||{}).textContent||'';o.current=f.querySelectorAll('.fdot[aria-current="page"]').length;o.inert=f.querySelectorAll('.fcard[inert]').length;o.ariaHidden=f.querySelectorAll('.fcard[aria-hidden="true"]').length;o.reachable=cards.length-o.inert;prev.click();o.wrapped=at()===was;if(play){var lab=play.getAttribute('aria-label');next.click();prev.click();o.stillPaused=play.getAttribute('aria-label')===lab;play.click();}return JSON.stringify(o)})()"""


JS_CONTRAST = """(function(){var lin=function(v){v/=255;return v<=0.03928?v/12.92:Math.pow((v+0.055)/1.055,2.4)};var lum=function(c){return 0.2126*lin(c[0])+0.7152*lin(c[1])+0.0722*lin(c[2])};var parse=function(s){var m=s&&s.match(/[\\d.]+/g);  if(!m)return null;return [+m[0],+m[1],+m[2],m.length>3?+m[3]:1]};var bgOf=function(el){var stack=[];var n=el;  while(n&&n.nodeType===1){var c=parse(getComputedStyle(n).backgroundColor);    if(c&&c[3]>0){stack.push(c); if(c[3]>=1)break;} n=n.parentElement;}  var out=[15,15,18];  for(var i=stack.length-1;i>=0;i--){var c=stack[i];    out=[0,1,2].map(function(k){return c[3]*c[k]+(1-c[3])*out[k]});}  return out};var bad=[];var seen=0;document.querySelectorAll('.detail *,.sw-global *,.sw-footer *').forEach(function(el){  if(el.closest('.btn,.ghcta,.sw-cta,.sw-mobile-cta')) return;  var txt='';  for(var i=0;i<el.childNodes.length;i++){    if(el.childNodes[i].nodeType===3) txt+=el.childNodes[i].textContent;}  txt=txt.trim(); if(txt.length<6) return;  var r=el.getBoundingClientRect(); if(!r.width||!r.height) return;  var cs=getComputedStyle(el);  if(cs.visibility==='hidden'||parseFloat(cs.opacity)<0.6) return;  var fg=parse(cs.color); if(!fg) return;  var bg=bgOf(el);  if(fg[3]<1){fg=[0,1,2].map(function(k){return fg[3]*fg[k]+(1-fg[3])*bg[k]});}  var L1=lum(fg),L2=lum(bg);  var ratio=(Math.max(L1,L2)+0.05)/(Math.min(L1,L2)+0.05);  var px=parseFloat(cs.fontSize);  var wt=parseInt(cs.fontWeight,10)||400;  var large=(px>=24)||(px>=18.66&&wt>=700);  var need=large?3:4.5;  seen++;  if(ratio<need-0.01) bad.push({t:txt.slice(0,32),px:px,    r:Math.round(ratio*100)/100,need:need});});return JSON.stringify({checked:seen,bad:bad.slice(0,6),n:bad.length})})()"""
...
JS_STACK = """(function(){var live=document.querySelector('a.buildcard:not(.planned)[style*="--sheets"]');var planned=document.querySelector('a.buildcard.planned');var o={live:!!live,planned:!!planned};if(live){var sh=live.querySelector('.sheet');var cs=getComputedStyle(sh);o.liveSheets=cs.boxShadow!=='none';o.liveEdge=cs.borderTopColor;o.liveEdgeSet=cs.borderTopColor!==getComputedStyle(document.body).borderTopColor;o.outerShadow=getComputedStyle(live).boxShadow==="none";}if(planned){var ps=getComputedStyle(planned.querySelector('.sheet'));o.plannedSheets=ps.boxShadow!=='none';o.plannedDashed=ps.borderTopStyle==='dashed';}return JSON.stringify(o)})()"""
...
JS_TOGGLE = """(function(){var b=document.getElementById('pvtog');if(!b)return JSON.stringify({skip:true});var paint=function(){return [].slice.call(document.querySelectorAll('.card,.buildcard')).filter(function(c){return getComputedStyle(c).display!=='none'}).length};var stale=function(){var bad=[];document.querySelectorAll('details.cat').forEach(function(g){g.querySelectorAll('.tgroup').forEach(function(tg){var k=[].slice.call(tg.querySelectorAll('.card')).filter(function(c){return getComputedStyle(c).display!=='none'}).length;var strip=g.querySelector('.tgs .tg[data-g="'+tg.dataset.g+'"]');if(!strip)return;var shown=strip.hidden?0:parseInt(strip.querySelector('.cn').textContent,10);if(shown!==k)bad.push(tg.dataset.g+':'+shown+'!='+k);});});return bad};var hid=function(){return !b.checked};var wasHid=hid();if(wasHid)b.click();var before=paint();b.click();var after=paint();var lab=b.closest('label');var ok1=!!lab&&/Show hidden/.test(lab.textContent);var badStrip=stale();b.click();var ok2=!!b.closest('label')&&/Show hidden/.test(b.closest('label').textContent);if(wasHid)b.click();return JSON.stringify({before:before,after:after,total:document.querySelectorAll('.card,.buildcard').length,hasSwitch:ok1&&ok2,defaultHidden:wasHid,staleStrip:badStrip.slice(0,4)})})()"""
...
def main(argv):
    ...
    try:
        for w, h in VIEWPORTS:
            ...
            if not r.get("skip"):
                ...
                if r.get("stepPx") is not None and abs(r["stepPx"] - r["expectedStep"]) > 1:
                    fail(w, "carousel", "one page moved the track %.1fpx; the viewport's content "
                                        "width plus the gap is %.1fpx, so pages drift"
                                        % (r["stepPx"], r["expectedStep"]))
            ...
            # the stack: a live build touching more than one category paints
            # its sheets on the .sheet, not on the card, and a planned build
            # is a dashed sheet with nothing under it
            r = evaljs(JS_STACK)
            if r.get("live"):
                if not r.get("liveSheets"):
                    fail(w, "stack", "a live build touching several categories "
                                     "paints no sheets behind it")
                if not r.get("outerShadow"):
                    fail(w, "stack", "the sheets paint on the card as well as "
                                     "the sheet, so the grid cell shows them")
            if r.get("planned"):
                if r.get("plannedSheets"):
                    fail(w, "stack", "a planned build paints sheets; only a "
                                     "build that exists is a stack")
                if not r.get("plannedDashed"):
                    fail(w, "stack", "a planned build's sheet is not dashed")
            ...
            if not r.get("skip"):
                ...
                if not r.get("hasSwitch"):
                    fail(w, "toggle", "the Show hidden checkbox lost its label "
                                      "after clicking")
                if not r.get("defaultHidden"):
                    fail(w, "toggle", "planned rows are shown by default; they are "
                                      "hidden until the reader asks")
                if r.get("staleStrip"):
    ...
    print("QC ok: overflow, overlap, click, toggle, tabs, banner, featured, "
          "carousel, taskgroup, divider, arrows, stack, sidescroll, deadlink, replay, contrast at "
