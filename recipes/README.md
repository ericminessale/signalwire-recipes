...
43 of 55 folders are written and verified. The rest are planned and carry a stub.
...
`call-an-mcp-server-from-a-live-call`, `expose-an-agent-as-an-mcp-server`, `governed-intake-agent`, `hand-off-from-ai-to-a-human-agent`, `handle-call-status-callbacks`, `measure-voice-ai-latency`, `put-an-agent-in-a-web-chat-widget`, `run-the-same-agent-over-text`, `sms-support-desk`, `take-a-payment-without-the-model-seeing-the-card`, `verify-a-webhook-signature`, `voice-support-line`
