...
Claim: `enable_vision: true` in `ai.params` lets the agent read the caller's
camera through the platform's `get_visual_input` function, `vision_model`
picks the model, and an internal filler for `get_visual_input` covers the
look.

Proof: render the agent's SWML. `ai.params` carries `enable_vision: true` and
`vision_model: gpt-4o-mini`, and the document validates. The bundled schema
documents `enable_vision` with the sentence naming `get_visual_input`, lists
`gpt-4o-mini` among `vision_model`'s allowed values, and lists
`get_visual_input` among the internal fillers. `SWAIG.internal_fillers`
...
def main():
    ...
    assert params["vision_model"] == "gpt-4o-mini", params
    ...
    assert "video call" in json.dumps(ai["prompt"]["pom"]), ai["prompt"]
    ...
    allowed = {alt["const"] for alt in props["vision_model"]["anyOf"] if "const" in alt}
    assert allowed == {"gpt-4o-mini", "gpt-4.1-mini", "gpt-4.1-nano"}, allowed
    assert params["vision_model"] in allowed
    ...
    print(f"ok: ai.params enable_vision=true, vision_model={params['vision_model']}; "
          f"SWAIG.internal_fillers covers get_visual_input with {len(LOOKING)} phrases; the schema "
          f"names get_visual_input in enable_vision and among the fillers, not the native functions")
