...
Claim: one number reaches sales, support or billing through the default
context, each persona is an isolated context, and each persona's step names
only its own tools, so the other desks' tools are structurally absent while it
is on the line.
...
def main():
    ...
    assert field["type"] == "boolean" and "resets conversation history" in field["description"]
