...
.ctas{margin-left:auto;display:inline-flex;gap:8px;align-items:center;}
.ctas .ghcta{margin-left:0;}
.ghcta.ghost{background:transparent;color:var(--fg);box-shadow:inset 0 0 0 1px var(--line-2);}
.ghcta.ghost:hover{filter:none;background:rgba(255,255,255,.05);box-shadow:inset 0 0 0 1px var(--line-3);}
@media (max-width:600px){.ghcta,.ctas{margin-left:0;}}
...
GH_PATH = ("M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 "
           ...
           "2-.27s1.36.09 2 .27c1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12"
...
def gh_cta(r):
    ...
    # A build that is deployed somewhere gets "Try it live" as the primary
    # action and the repository steps back to a ghost button: trying the thing
    # is the better first click, and the page has one primary.
    demo = r.get("demo_url")
    if demo:
        return ('<span class="ctas">'
                '<a class="ghcta" href="%s">Try it live</a>'
                '<a class="ghcta ghost" href="%s">'
                '<svg viewBox="0 0 16 16" aria-hidden="true" focusable="false">'
                '<path fill="currentColor" d="%s"/></svg><span>%s</span></a></span>'
                % (esc(demo), esc(target), GH_PATH, esc(label)))
...
def build_md(r):
    ...
    if r.get("demo_url"):
        lines.append(f"- **Live demo:** {r['demo_url']}")
